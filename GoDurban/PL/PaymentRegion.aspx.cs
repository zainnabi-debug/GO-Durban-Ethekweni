...
{
    public partial class PaymentRegion : System.Web.UI.Page
    {
        ...
        private void LoadButtonVisible()
        {
            ...
            var noClickedList = db.NoOfClicksPerMonths.Where(x => x.ColumnName == "Region").ToList();
        ...
        private void LoadBtnClickedOnce()
        {
            ...
            if (noClickedList.Count() > 0)
            {
                ...
                if (noClicked.MonthClicked < DateTime.Now.Month && noClicked.ColumnName == "Region")
                {
                    lblSent.Visible = false;
                    //btnExportTo.Enabled = true;
                }
        ...
        private bool ValidateYesNo()
        {
            ...
            var cell = db.Payments.ToList().FindAll(x => x.IsPaymentSent == "Yes");
            if (cell.Count > 0)
            {
                lblSent.Visible = true;
                //btnExportTo.Enabled = false;
        ...
        private void LoadData()
        {
            ...
            var list = (from a in db.Payments
                        ...
                        where (a.AccountNo.StartsWith("R"))
            ...
            try
            {
                ...
                for (int i = 0; i < gvPayment.PageCount; i++)
                {
                    ...
                    foreach (GridViewRow row in gvPayment.Rows)
                    {
                        ...
                        //var payreg = db.Payments.ToList().Find(x => x.Amount == Convert.ToDecimal(8000.00) && x.IsPaymentSent == "No");
                        ...
                        {
                            int ownassregid = Convert.ToInt16(payreg.RegionID);
                            ...
                            using (OdbcConnection con = new OdbcConnection(ODBCconstr))
                            {
                                    ...
                                    "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", con))
                                {
                                    try
                                    {
                                        ...
                                        var payId = db.Payments.ToList().Find(x => x.RegionID == ownassregid && x.PaymentID == id);
                                        ...
                                        var numOfClicks = db.NoOfClicksPerMonths.ToList().FindAll(x => x.MonthClicked == DateTime.Now.Month && x.ColumnName == "Region");
